...
import * as donHangService from "../service/HoaDonService.js"
...
function UpdateHoaDon() {
    ...
    useEffect(() => {
        const updateDonHang=async ()=>{
            const hoaDon=await donHangService.findIdHoaDon(id);
            if(hoaDon){
                setHoaDon({
                    ...
                    thang: hoaDon.thang,
                    khachHang:  JSON.stringify(hoaDon.khachHang)
                });
            }else {
    ...
    const handleSubmit = async (value, {isSubmitting, resetForm}) => {
        ...
        const success=await donHangService.updateHoaDon(id,value);
        if(success){
